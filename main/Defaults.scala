...
object Defaults extends BuildCommon
{
	...
	def projectCore: Seq[Setting[_]] = Seq(
		...
		logManager := LogManager.defaults(extraLoggers.value),
	...
	def compileInputsSettings: Seq[Setting[_]] =
		Seq(compileInputs := {
			...
			Compiler.inputs(cp, sources.value, classDirectory.value, scalacOptions.value, javacOptions.value, maxErrors.value, compileOrder.value)(compilers.value, compileIncSetup.value, streams.value.log)
		})
...
object Classpaths
{
	...
	def sbtClassifiersTasks = inTask(updateSbtClassifiers)(Seq(
		...
		ivyConfiguration := new InlineIvyConfiguration(ivyPaths.value, externalResolvers.value, Nil, Nil, offline.value, Option(lock(appConfiguration.value)), checksums.value, streams.value.log),
		ivySbt <<= ivySbt0,
